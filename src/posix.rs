...
//! possible. The aim is to take little assumptions are require as little configuration as
...
//!
//! Additionally, it could function well as an initial reasoner as a user adopts Brane on their
//! systems, since it could use already set permissions of their current systems to
...
//! reasoner. The policy reasoner will then evaluate if all files are properly accessible and
//! return a verdict.
//!
//! ## Limitations
//!
...
//! Right now there is support in the code for multiple locations with each their own user
//! mapping, as different sites will often comprise of different network shares. But as it is
//! unclear right now how the volumes will be mounted on the reasoner container, it is hard to tell
//! how such an implementation is best designed.
//!
//! - Right now, this reasoner is both a module and a binary, but it should probably just be a
//! binary. At this moment however, documenting binaries is tricky in rust. As soon as we find a better
...
//! implementations.
//!
//! - LDAP / Active Directory support: Currently the Brane user to uid / gid mapping is embedded in
...
//! far less common that the regular POSIX permission it attempts to extend, but usage is also far
//! from uncommon and potenial users of Brane, and more specifically the policy reasoner, could have
//! used these ACLs on their existing file systems.
//!
